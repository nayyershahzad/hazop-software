...
interface GeminiInsightsPanelProps {
  ...
  onAddRecommendation?: (recommendation: AISafeguardSuggestion) => void;
...
export const GeminiInsightsPanel = ({
    ...
    try {
      ...
      switch (insightType) {
        ...
        case 'consequences':
          ...
          break;
        case 'recommendations':
          setSuggestedRecommendations(response.data);
          break;
      }
    ...
    try {
      ...
      switch (type) {
        ...
        case 'consequences':
          ...
          break;
        case 'recommendations':
          endpoint = `${API_URL}/api/gemini/apply-suggestions/recommendations`;
          payload = {
            deviation_id: deviation.id,
  ...
  return (
    <div className={`fixed bottom-0 left-0 ${isCollapsed ? 'w-56' : 'w-96'} bg-white rounded-tr-lg shadow-2xl border-2 border-purple-300 transition-all duration-300 z-50 ${isCollapsed ? 'h-14' : 'max-h-[600px]'}`}>
      {/* Header */}
      <div className="bg-gradient-to-r from-purple-600 to-blue-600 px-4 py-3 rounded-tr-lg flex justify-between items-center text-white cursor-pointer" onClick={toggleExpanded}>
        ...
        <div className="p-4">
          ...
          {!isLoading && insightType === 'recommendations' && !selectedConsequenceId && (
            <div className="bg-yellow-50 border border-yellow-200 rounded p-3 mb-3">
              <p className="text-sm text-yellow-800 flex items-center">
                <span className="mr-2">⚠️</span>
                <span><strong>Note:</strong> Select a Consequence from the list above before adding Recommendations. Click on a consequence to highlight it.</span>
              </p>
            </div>
          )}
          ...
          {!isLoading && (
            <div className="max-h-64 overflow-y-auto">
              ...
              {insightType === 'recommendations' && suggestedRecommendations.length === 0 && (
                <p className="text-sm text-gray-500 italic text-center py-4">
                  No recommendation suggestions available. Click refresh to generate.
                </p>
              )}
              ...
              {insightType === 'recommendations' && suggestedRecommendations.map((suggestion, index) => (
                <div key={index} className="border mb-2 rounded overflow-hidden">
