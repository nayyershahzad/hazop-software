...
export const StudyDetail = () => {
  ...
  const [pendingDeviation, setPendingDeviation] = useState<Deviation | null>(null); // Track pending switch
  const [showNodeModal, setShowNodeModal] = useState(false);
  const [showDeviationModal, setShowDeviationModal] = useState(false);
  const [showDuplicateModal, setShowDuplicateModal] = useState(false);
  const [showUnsavedWarning, setShowUnsavedWarning] = useState(false); // Confirmation dialog
  const [nodeToDuplicate, setNodeToDuplicate] = useState<Node | null>(null);
  const [editingNode, setEditingNode] = useState<Node | null>(null);
  const [hasUnsavedChanges, setHasUnsavedChanges] = useState(false); // Track changes from child
  const [expandedDeviations, setExpandedDeviations] = useState<Set<string>>(new Set()); // Track expanded deviations
  ...
  useEffect(() => {
    if (selectedNode) {
      ...
      // Clear deviation selection when switching nodes
      setSelectedDeviation(null);
      setExpandedDeviations(new Set());
  ...
  // Handle deviation switch with unsaved changes check
  const handleDeviationClick = (deviation: Deviation) => {
    ...
    if (hasUnsavedChanges && selectedDeviation && selectedDeviation.id !== deviation.id) {
      ...
      // No unsaved changes or same deviation, switch directly
  ...
  const confirmSwitchWithoutSaving = () => {
    if (pendingDeviation) {
      ...
      // Auto-expand new deviation and collapse others
      setExpandedDeviations(new Set([pendingDeviation.id]));
            ...
            <div className="bg-white rounded-lg shadow p-4">
              {selectedNode ? (
                <>
                  ...
                  {/* Deviations as Collapsible Cards */}
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <h3 className="font-semibold text-gray-900">
                                    {dev.parameter} / {dev.guide_word}
                                  </h3>
                                  <p className="text-sm text-gray-600 mt-1">
                                    {dev.deviation_description}
                                  </p>
                                </div>
                              </div>
                            </div>
